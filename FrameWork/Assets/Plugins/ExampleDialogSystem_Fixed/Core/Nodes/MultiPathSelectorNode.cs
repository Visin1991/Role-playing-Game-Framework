...
public class MultiPathSelectorNode : BaseDialogNode
{
	...
	public override BaseDialogNode Input(int inputValue)
	{
		...
		if (Outputs[nodeIndex].GetNodeAcrossConnection() != default(Node)) // default(Node) will return null, Because Node is a reference Type。 default(Int32) will return 0, because it is a value type.
	...
	private int GetNodeIndexFor(float value)
	{
		...
		return(from option in _options where option.MinValue <= value && value <= option.MaxValue select option.NodeOutputIndex).FirstOrDefault();
	}
